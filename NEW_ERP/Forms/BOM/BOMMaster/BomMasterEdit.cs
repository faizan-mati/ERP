...
{
    public partial class BomMasterEdit : EditFormTemplate
    {
        public BomMasterEdit()
        {
            InitializeComponent();
        }

        private void BomMasterEdit_Load(object sender, EventArgs e)
        {
            BomMasterIdShow();
        ...
        //======================================= SALE ORDER SELECTED (CHANGE PRODUCT LIST) =======================================

        private void SaleOrderBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (SaleOrderBox.SelectedValue != null && int.TryParse(SaleOrderBox.SelectedValue.ToString(), out int saleOrderId))
            {
                LoadProducts(saleOrderId);
            }
        }
        ...
        private void UpdateBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please select a BOM ID to update.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (validation())
            {
                try
                {
                    ...
                    string productId = ProductBox.SelectedValue.ToString();
                    int saleOrderId = Convert.ToInt32(SaleOrderBox.SelectedValue);
                    string versionNo = txtVersionNo.Text.Trim();

                    using (SqlConnection conn = new SqlConnection(AppConnection.GetConnectionString()))
                    {
                        using (SqlCommand cmd = new SqlCommand("sp_UpdateBOMMaster", conn))
                        {
                            ...
                            {
                                MessageBox.Show("No record found with that BOM ID.", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            }
                        }
                    }
                }
